import type { PublishResponse } from '@artifaq/shared';

const API_BASE = import.meta.env.VITE_API_BASE ?? '/__api';
...
export async function publishArtifact(input: PublishInput): Promise<PublishResponse> {
  ...
  if (input.turnstileToken) headers['CF-Turnstile-Token'] = input.turnstileToken;
